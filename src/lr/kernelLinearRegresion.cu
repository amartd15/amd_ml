...
__host__ void linearRregresionKernel(
    ...
    float mse = 0.0f;
    //tensor* mse_aux = createTensor(0.0f, 1, 1);

    //We create shared variables
    float aux = 0.0f;

    //float* smse             = createSharedPointer(aux1);
    float* smse_aux         = createSharedPointer(aux);
    //float* slearning_rate   = createSharedPointer(); 
    ...
    do{
        ...
        // lr_reset_gradent<<<numBlocksParam, numThreads>>>(
        //     gradient->data_d,
        //     n_param
        // );
        ...
        (++iter < n_iter)                                                   /*&&
        (lr_compare_mse(error, &mse, smse_aux, hiperparameters))*/  
    );
...
__host__ bool lr_compare_mse(tensor* error, float* mse, float* mse_aux, lr_hiperparameters* param){
    ...
    //copyMemory(mse_aux, DEVICE_TO_HOST);
